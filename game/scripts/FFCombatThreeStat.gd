...
const VisualsThree = preload("res://scripts/FFTacticalVisuals.gd")
    ...
    if visible_cells.has(cell):
        ...
        if zi != -1:
            if bool(player.weapon.gun): shoot(zi)
                ...
                msg = "Too far for %s." % player.weapon.name; queue_redraw()
            return
...
func try_move(dir: Vector2i):
    ...
    var noise := ThreeStatRules.sprint_noise(agility) if sprinting else (ThreeStatRules.stealth_noise(agility) if player.crouched else 18)
    ...
    var base_cost := TimeThree.movement_cost(player, false)
    var cost := ThreeStatRules.sprint_move_cost(agility, base_cost) if sprinting else (ThreeStatRules.stealth_move_cost(agility, base_cost) if player.crouched else ThreeStatRules.normal_move_cost(agility, base_cost))
    commit_action(cost)

func stealth_attack(z) -> bool:
    if not bool(player.crouched) or z.state == "CHASE":
        return false
    var to_player: Vector2i = player.pos - z.pos
    var positional := dominant(to_player) == -z.facing or not zombie_sees_actor(z, player)
    if not positional:
        return false
    var agility := int(player.get("skills", {}).get("Agility", 0))
...
func melee(target: Vector2i):
    ...
    var chance := ThreeStatRules.attack_chance(combat, attack_penalty(player), float(player.weapon.get("accuracy", 0.0)), stealth)
    if rng.randf() <= chance:
        var d := ThreeStatRules.melee_damage(player.weapon, combat, stealth, rng)
        zombies[zi].hp -= d
...
func shoot(i: int):
    ...
    var chance := ThreeStatRules.attack_chance(combat, attack_penalty(player) + range_penalty, float(player.weapon.get("gaccuracy", 0.0)), false)
    stats.shots += 1
    _flash_muzzle(player.pos, player.facing)
    if rng.randf() <= chance:
        var d := ThreeStatRules.gun_damage(player.weapon, combat, rng)
        zombies[i].hp -= d
        ...
        if int(player.weapon.get("hands", 1)) == 2 and player.weapon.name == "Shotgun":
...
func zombie_attack(i: int, target_actor: Dictionary):
    ...
    var hit := BalanceThree.zombie_hit_chance(target_actor)
    if target_actor.controlled and guarded:
        target_actor["guarding"] = false
    if rng.randf() <= hit:
        var damage_range := BalanceThree.zombie_damage_range(str(zombies[i].get("mass", "MED")))
...
func draw_hud():
    ...
    var class_label := str(player.weapon.get("class_label", "1H MELEE"))
    var weapon_line := "%s | %s" % [class_label, str(player.weapon.name)]
    ...
    draw_button(btn_crouch,"STEALTH",bool(player.crouched),10)
    ...
    var has_portable_light := LightingThree.item_emits_light(portable_light)
    ...
    draw_button(btn_sprint,"SPRINT",bool(player.get("sprinting", false)),10)
    var agility := int(player.skills.get("Agility", 0))
    var base_step := TimeThree.movement_cost(player, false)
    var step_cost := ThreeStatRules.sprint_move_cost(agility, base_step) if bool(player.get("sprinting", false)) else (ThreeStatRules.stealth_move_cost(agility, base_step) if player.crouched else ThreeStatRules.normal_move_cost(agility, base_step))
